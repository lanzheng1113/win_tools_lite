...
BOOL GetCpuId()
{
    ...
    if( !bException )
    {
		...
		//printf("CPU id=%s-%d",szCpu,uCpuID);
		return TRUE;
    }else{
		//printf("getcpu id failed");
		return FALSE;
	}
}
...
/*	printf("\nChar:\n");
	...
	printf("\nHex:\n");
	for(UINT ii = 0; ii < uSystemInfoLen; ii++)
		printf("%x ", szSystemInfo[ii], szSystemInfo[ii]);

	printf("\nD:\n");
	for(UINT iii = 0; iii < uSystemInfoLen; iii++)
		printf("%u", szSystemInfo[iii], szSystemInfo[iii]);*/
	unsigned char * RetSerialN=NULL;  
	string ls;
	
	printf("\ntotal len=%d",uSystemInfoLen);
	 
	printf("\nChar:\n");
	for(UINT i = 0; i < uSystemInfoLen; i++)
		printf("%c", szSystemInfo[i]);
		
	printf("\nHex:\n");
	for(UINT ii = 0; ii < uSystemInfoLen; ii++)
		printf("%x ", szSystemInfo[ii]);
		
	printf("\nD:\n");
		
for(UINT iiii = 0; iiii < uSystemInfoLen; iiii++)   
	printf("%d","RetSerialN[iiii]");

	printf("\nU:\n");
for(UINT iii = 0; iii < uSystemInfoLen; iii++)
{
    
		printf("%u", szSystemInfo[iii]);	
...
char tmp[4096];
int offset=0;
for(UINT qiii = 0; qiii < uSystemInfoLen; qiii++)
{
   
	offset += sprintf(tmp + offset, "%u", szSystemInfo[qiii]);

	
	//ls+= szSystemInfo[qiii];
	//ls+=(char*)tmp;
	
	//	printf("%c", tmp);
  //char[]   cha=strTemp.ToCharArray();   
